...
<x-layouts.dashboard title="Presensi" description="Lihat data presensi dari semua cabang.">
    <section class="space-y-6">
        <div class="flex justify-end">
            <form method="GET" action="{{ route('leader.attendance') }}" class="grid w-full gap-2 sm:w-auto sm:grid-cols-2 xl:grid-cols-[200px_160px_160px_auto]">
                <div class="relative">
            ...
            <table class="w-full min-w-[920px] text-left">
                <thead>
                    <tr class="bg-white text-sm text-slate-950">
                        ...
                        <th class="px-6 py-5 font-bold">Santri Hadir</th>
                ...
                <tbody>
                    @forelse ($attendances as $attendance)
                        <tr class="{{ $loop->odd ? 'bg-slate-100' : 'bg-white' }}">
                            <td class="px-6 py-5 text-sm">{{ $attendances->firstItem() + $loop->index }}</td>
                            <td class="px-6 py-5 text-sm">{{ $attendance->attendance_date->format('d M Y') }}</td>
                            <td class="px-6 py-5 text-sm">{{ $attendance->branch?->name ?: $attendance->group?->branch?->name ?: '-' }}</td>
                            <td class="px-6 py-5 text-sm">{{ $attendance->group?->name ?: '-' }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-8 text-center text-sm text-slate-500">Belum ada data presensi.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
