<x-layouts.dashboard title="Data Wali Santri" description="Kelola data wali santri dan hubungan dengan santri.">
    ...
    <section class="space-y-6">
        <div class="flex justify-end">
            <form method="GET" action="{{ route('admin.guardians') }}" class="grid w-full gap-2 sm:grid-cols-2 xl:w-auto xl:grid-cols-[200px_160px_auto]">
                <div class="relative">
                        ...
                        <tr class="{{ $loop->odd ? 'bg-slate-100' : 'bg-white' }}">
                            ...
                            <td class="px-6 py-5">
                                <div class="flex items-center gap-4">
                                    <button type="button" onclick="openDialog('view-guardian-{{ $guardian->id }}')" class="cursor-pointer text-slate-900 hover:text-blue-950" aria-label="Lihat wali"><x-icon name="eye" /></button>
                                    <a href="{{ route('admin.guardians.edit', $guardian) }}" class="cursor-pointer text-slate-900 hover:text-blue-950" aria-label="Edit wali"><x-icon name="pencil" /></a>
                                    <form method="POST" action="{{ route('admin.guardians.destroy', $guardian) }}" onsubmit="return confirm('Hapus wali santri ini?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="cursor-pointer text-slate-900 hover:text-red-600" aria-label="Hapus wali"><x-icon name="trash" /></button>
                                    </form>
                                </div>
                            </td>
                        </tr>
