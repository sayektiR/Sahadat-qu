...
<x-layouts.dashboard :title="$pageTitle" description="Atur periode, kelompok, jam belajar, dan materi setiap hari.">
        ...
        <section class="w-full rounded-lg border border-slate-200 bg-white p-6 shadow-sm lg:p-8">
            <div class="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
                <div>
                    ...
                    <input name="start_date" type="date" value="{{ old('start_date', $schedule->start_date?->format('Y-m-d')) }}" required class="mt-2 h-11 w-full rounded-md border border-slate-300 bg-slate-100 px-3 text-sm outline-none focus:border-blue-950 focus:ring-2 focus:ring-blue-950/10">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-900">Tanggal Selesai</label>
                    <input name="end_date" type="date" value="{{ old('end_date', $schedule->end_date?->format('Y-m-d')) }}" required class="mt-2 h-11 w-full rounded-md border border-slate-300 bg-slate-100 px-3 text-sm outline-none focus:border-blue-950 focus:ring-2 focus:ring-blue-950/10">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-900">Jumlah Pertemuan</label>
                    <input name="total_meetings" type="number" min="1" value="{{ old('total_meetings', $schedule->total_meetings) }}" required placeholder="Jumlah pertemuan" class="mt-2 h-11 w-full rounded-md border border-slate-300 bg-slate-100 px-3 text-sm outline-none focus:border-blue-950 focus:ring-2 focus:ring-blue-950/10">
                </div>
                <div>
                ...
                <div class="md:col-span-3">
                    ...
                    <select name="period_id" required class="mt-2 h-11 w-full rounded-md border border-slate-300 bg-slate-100 px-3 text-sm outline-none focus:border-blue-950 focus:ring-2 focus:ring-blue-950/10">
                        <option value="">Pilih periode</option>
                        @foreach ($periods as $period)
                            <option value="{{ $period->id }}" @selected((string) old('period_id', $schedule->period_id) === (string) $period->id)>{{ $period->name }} - {{ $period->semester }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
                        ...
                        <div class="space-y-3" data-slots-for="{{ $day }}" data-next-index="{{ $slotCount }}">
                            @for ($slot = 0; $slot < $slotCount; $slot++)
                                ...
                                <div class="grid gap-2 rounded-md border border-slate-200 bg-white p-3" data-schedule-slot>
                                    ...
                                    <select name="details[{{ $day }}][{{ $slot }}][subject_id]" class="h-10 rounded-md border border-slate-300 bg-white px-3 text-sm outline-none focus:border-blue-950 focus:ring-2 focus:ring-blue-950/10">
    ...
    <template id="schedule-slot-template">
        <div class="grid gap-2 rounded-md border border-slate-200 bg-white p-3" data-schedule-slot>
            ...
            <select data-slot-subject class="h-10 rounded-md border border-slate-300 bg-white px-3 text-sm outline-none focus:border-blue-950 focus:ring-2 focus:ring-blue-950/10">
                <option value="">Pilih mapel</option>
                @foreach ($subjects as $subject)
