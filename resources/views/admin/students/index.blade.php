<x-layouts.dashboard title="Data Santri" description="Kelola data santri berdasarkan kelompok dan wali santri.">
    ...
    <section class="space-y-6">
        <div class="flex justify-end">
            <form method="GET" action="{{ route('admin.students') }}" class="grid w-full gap-2 sm:grid-cols-2 xl:w-auto xl:grid-cols-[200px_160px_auto]" id="filterForm">
                <div class="relative">
                        ...
                        <tr class="{{ $loop->odd ? 'bg-slate-100' : 'bg-white' }}">
                            ...
                            <td class="px-6 py-5">
                                <div class="flex items-center gap-4">
                                    ...
                                    <a href="{{ route('admin.students.edit', $student) }}" class="cursor-pointer text-yellow-500 hover:text-yellow-700" aria-label="Edit santri"><x-icon name="pencil" /></a>
                                    <form method="POST" action="{{ route('admin.students.destroy', $student) }}" class="delete-form">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="delete-btn cursor-pointer text-red-500 hover:text-red-700" aria-label="Hapus santri"><x-icon name="trash" /></button>
                                    </form>
                                </div>
                            </td>
                        </tr>
